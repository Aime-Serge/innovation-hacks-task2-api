...
<main class="wrap">
  ...
  <h1>__TITLE__</h1>
  <p class="lead">REST API for users, projects and tasks, built to the Task 2 brief. Every requirement below can be verified live, from this page.</p>
  <div class="meta">
  ...
  <p class="sub">Runs real requests against this server and shows the result of each. It creates one temporary user with a project and a task, then deletes them.</p>
  ...
  <footer>
    <p>Spec: <a href="/openapi.json">/openapi.json</a> · Free tier: the first request after inactivity can take up to a minute. Data is in memory and resets on restart.</p>
  </footer>
</main>
<script>
(function () {
  ...
  var steps = [
    ...
    ["Reject a duplicate email", async function (c) {
      ...
      return [r.status === 409 && code(r) === "conflict", "409 conflict"];
    }],
    ...
    ["Reject a project with an unknown owner", async function () {
      ...
      return [r.status === 404, "404 not_found"];
    }],
    ["Create a project and a task", async function (c) {
      ...
      c.task = t.body;
      return [p.status === 201 && t.status === 201 && t.body.status === "todo", "201 · new tasks start as todo"];
    }],
    ["Change a task's status", async function (c) {
      var r = await api("PATCH", "/tasks/" + c.task.id + "/status", { status: "done" });
      return [r.status === 200 && r.body.status === "done", "todo → done"];
    }],
    ["Reject an invalid status", async function (c) {
      var r = await api("PATCH", "/tasks/" + c.task.id + "/status", { status: "archived" });
      return [r.status === 422, "422 validation_error"];
    }],
    ...
    ["Wrong HTTP method uses it too", async function () {
      var r = await api("POST", "/health");
      return [r.status === 405 && code(r) === "method_not_allowed", "405 method_not_allowed"];
    }]
  ];
